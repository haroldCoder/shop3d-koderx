...
export default function CreateModel() {
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSizeBytes) {
      toast.error("file is superior to 10MB");
    }
    else {
        setFile(file);
        setModel(URL.createObjectURL(file))
        console.log(URL.createObjectURL(new Blob([await convertFileToBase64(file)])));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setModeluri(await uploadToCloudinary(await convertFileToBase64(file), `${user?.fullName}/${data.name}`.toString()));
